...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="APPVERSION">1.50.1</xsl:variable>
	<xsl:variable name="SITEHOST">www.pcjs.org</xsl:variable>
	<xsl:variable name="BGNDCOLOR">#FAEBD7</xsl:variable>

	<xsl:template name="componentStyles">
		<link rel="stylesheet" type="text/css" href="/versions/c1pjs/1.50.1/components.css"/>
	</xsl:template>
		...
		<div id="{$id}" class="{$componentClass}" style="{$width}{$height}{$pos}{$left}{$top}{$padding}">
			...
			<xsl:if test="$component = 'machine'">
				...
				<div class="{$APPCLASS}-copyright">
					<a href="http://{$SITEHOST}/{$APPCLASS}" target="_blank">C1Pjs</a> v<xsl:value-of select="$APPVERSION"/> © 2012-2019 by <a href="http://twitter.com/jeffpar" target="_blank">@jeffpar</a>
				</div>
