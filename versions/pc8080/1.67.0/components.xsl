...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="componentStyles">
		<link rel="stylesheet" type="text/css" href="/versions/pc8080/1.67.0/components.css"/>
	</xsl:template>
		...
		<div id="{$id}" class="{$componentClass}" style="{$width}{$height}{$pos}{$left}{$top}{$padding}" data-value="{$machineParms}">
			...
			<xsl:if test="$component = 'machine'">
				...
				<div class="{$CSSCLASS}-copyright">
					<a href="http://{$SITEHOST}" target="_blank"><xsl:value-of select="$APPNAME"/></a> v<xsl:value-of select="$APPVERSION"/> © 2012-2018 by <a href="https://jeffpar.com" target="_blank">@jeffpar</a>
				</div>
